	...
	if(argc<2)
{
	...
	// Beware of automatic prefetch in modern CPUs - it can often detect strided accesses. So try a random access pattern
			...
			for(j=0;j<ncolumns;j++)
			{
				for(k=0;k<nrows;k++)
				{
				  int ii = rand() % nrows;
					...
					array3[ii][jj]=array3[ii][jj]+ (array1[ii][kk]*array2[kk][jj]);
